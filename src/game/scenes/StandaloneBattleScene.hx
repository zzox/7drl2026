...
class StandaloneBattleScene extends ButtonScene {
    ...
    override function create () {
        ...
        final p1 = room.actors[0];
        final p2 = room.actors[1];
...
// #end
    ...
    override function update (delta:Float) {
        ...
        if (Game.keys.justPressed(KeyCode.R)) {
            game.changeScene(new StandaloneBattleScene(room.actors[0].dna, room.actors[1].dna));
        }
    ...
    override function render (g2:Graphics, clears:Bool) {
        ...
        roomRender(g2, 100, 32, room, particles);
    ...
    function showDamage (dna:Dna, thingType:ThingType, amount:Int) {
        final otherGuy = if (room.actors[0].dna.id == dna.id) {
            room.actors[1].dna;
        } else {
            room.actors[0].dna;
        }
