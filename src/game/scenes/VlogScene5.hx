...
import game.util.Utils.getRandomItem;
...
class VlogScene5 extends ButtonScene {
    var page:Int = 0;
    var maxPages:Int = 0;
    ...
    override function create () {
        ...
        if (dnas == null) {
            ...
            final all:Array<DnaData> = [];

            for (_ in 0...256) {
                final list = [for (_ in 0...32) new Dna()];

                for (d in list) {
                    for (i in 0...d.genes.length) {
                        // change num here for strength
                        if (Math.random() < 0.9) {
                            d.genes[i] = getRandomItem(mutItems);
                        }
                    }
                }

                final rDnas:Array<DnaData> = list.map(d -> {
                    return {
                        w: 0, l: 0, t: 0, dna: d
                    }
                });
