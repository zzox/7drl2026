...
class UiScene extends ButtonScene {
    ...
    function makeTopButtons (sceneIndex:Int) {
        entities.push(new UiElement(0, 0, 16, 16, 4, 4, 12, 12, 60, 16, 25, Assets.images.ui));
        entities.push(new UiElement(320 - 60, 0, 16, 16, 4, 4, 12, 12, 60, 16, 25, Assets.images.ui));

        buttons.push(makeUiTextButton(60, 0, 40, 16, sceneIndex == 0 ? 37 : 16, 'BTTL', sceneIndex == 0 ? null : () -> {
            game.changeScene(new PreBattleScene());
        }));

        buttons.push(makeUiTextButton(100, 0, 40, 16, sceneIndex == 1 ? 37 : 16, 'SYNC', sceneIndex == 1 ? null : () -> {
            game.changeScene(new SyncScene());
        }));

        buttons.push(makeUiTextButton(140, 0, 40, 16, sceneIndex == 2 ? 37 : 16, 'RSTR', sceneIndex == 2 ? null : () -> {
            game.changeScene(new RosterScene());
        }));

        buttons.push(makeUiTextButton(180, 0, 40, 16, sceneIndex == 3 ? 37 : 16, 'SHOP', sceneIndex == 3 ? null : () -> {
